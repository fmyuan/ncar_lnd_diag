...
sub checkAnnualFile {
        if ($mode eq "clm2") {
                $annFile = $casedir.$caseid."_annT_".$yr_prnt.".nc";
                return(-1) if !-e $annFile ||  -z $annFile;
        }
        if ($mode eq "cam2") {
                $annFile = $casedir.$caseid."_annT_atm_".$yr_prnt.".nc";
                return(-1) if !-e $annFile ||  -z $annFile;
        }
        if ($mode eq "rtm") {
                $annFile = $casedir.$caseid."_annT_rtm_".$yr_prnt.".nc";
                return(-1) if !-e $annFile ||  -z $annFile;
        }
}
...
sub finalCheck {
  if ( $runClimo) {
    foreach $f (@reqFileListClimo) {
        ...
        $num = $castCtr+1;
        $flag = "climo_".$num;
        if ($meansFlag) {       # 1=On; 0=OFF
                next if ( $f eq "DJF_means" || $f eq "MAM_means" || $f eq "JJA_means" || $f eq "SON_means");
        }
        else {
           if (! -e $file && flag) {
