...
for $set (@setList)
{
  if( @status[$set-1]) { 
	if ($set == 5) { 
		system("cp $wkdir/set5_*.txt $webdir/set5/"); 
	} else {
		 ...
		 if ($plot_type eq "ps") {
			...
			if($fn eq "set3_reg_" || $fn eq "set6_reg_" || 
			   $fn eq "set7_ANN_" || $fn eq "set7_stat" || $fn  eq "set7_ocea" ) { 
			      ...
			      if($set == 2 && $compareModels) { 
			      	 $commandLine = "$convert $flags $file $fout"; 
			      } else { 
				     if($set == 1) {
				     	$commandLine = "$convert $smflags $file $fout"; 
			         }else { 
			         	$commandLine = "$convert $flags $file $fout"; 
			         }
			      }
			}
			#print "converting:  $file ==> $fout\n";
