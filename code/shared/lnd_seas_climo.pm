...
sub create_SEAS_climo_step1
{
  ...
  foreach $m (@monList) {
	...
	print( "processing $ifile to $ofile\n");
        if ($DEBUG) { print("ncra $atts $ifile $ofile\n"); }
        $err = system("ncra $atts $ifile $ofile\n");  die "ncra climo failed\n" if $err;
...
sub create_SEAS_climo_step2
{
	...
	if ( $weightAnnAvg) {
	   ...
	   system("rm $out $tmpfile");
           ...
           print("ncra $flags @DJF $ofileDJF\n") if $DEBUG;
           print("ncra $flags @MAM $ofileMAM\n") if $DEBUG;
           print("ncra $flags @JJA $ofileJJA\n") if $DEBUG;
           print("ncra $flags @SON $ofileSON\n") if $DEBUG;
           ...
           $err = system("ncra $flags @DJF $ofileDJF"); die "SEAS_climo DJF failed \n" if $err;
           $err = system("ncra $flags @MAM $ofileMAM"); die "SEAS_climo MAM failed \n" if $err;
           $err = system("ncra $flags @JJA $ofileJJA"); die "SEAS_climo JJA failed \n" if $err;
           $err = system("ncra $flags @SON $ofileSON"); die "SEAS_climo SON failed \n" if $err;
